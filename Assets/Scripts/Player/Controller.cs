...
public class Controller : MonoBehaviour
{
    ...
    [Header("GameObject/Component References")] [Tooltip("The Rigidbody2D component to use in \"Astroids Mode\".")]
    public Rigidbody2D myRigidbody;

    [Header("Movement Variables")] [Tooltip("The speed at which the player will move.")]
    public float moveSpeed = 10.0f;

    [Tooltip("The speed at which the player rotates in asteroids movement mode")]
    public float rotationSpeed = 60f;

    [Header("Input Actions & Controls")] [Tooltip("The input action(s) that map to player movement")]
    ...
    private void Start()
    {
        if (moveAction.bindings.Count == 0 || lookAction.bindings.Count == 0)
            Debug.LogWarning(
                "An Input Action does not have a binding set! Make sure that each Input Action has a binding set or the controller will not work!");
    }
    ...
    private void HandleInput()
    {
        ...
        if (moveAction.bindings.Count == 0)
            Debug.LogError(
                "The Move Input Action does not have a binding set! It must have a binding set in order for movement to happen!");
        var moveInput = moveAction.ReadValue<Vector2>();
        var horizontalMovement = moveInput.x;
        var verticalMovement = moveInput.y;
        var movementVector = new Vector3(horizontalMovement, verticalMovement, 0);
        // Move the player
    ...
    public Vector2 GetLookPosition()
    {
        ...
        if (aimMode != AimModes.AimForwards)
        {
            if (lookAction.bindings.Count == 0)
                Debug.LogError(
                    "The Look Input Action does not have a binding set! It must have a binding set in order for the player to look around!");
    ...
    private void MovePlayer(Vector3 movement)
    {
        ...
        if (movementMode == MovementModes.Astroids)
        {
            ...
            var newRotationEulars = transform.rotation.eulerAngles;
            var zAxisRotation = transform.rotation.eulerAngles.z;
            var newZAxisRotation = zAxisRotation - rotationSpeed * movement.x * Time.deltaTime;
