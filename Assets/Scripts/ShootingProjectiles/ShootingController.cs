...
{
    [Header("GameObject/Component References")] [Tooltip("The projectile to be fired.")]
    ...
    [Header("Firing Settings")] [Tooltip("The minimum time between projectiles being fired.")]
    ...
    [Header("Effects")] [Tooltip("The effect to create when this fires")]
    ...
    private void Start()
    {
        if (fireAction.bindings.Count == 0 && isPlayerControlled)
            Debug.LogWarning(
                "The Fire Input Action does not have a binding set but is set to be player controlled! Make sure that it has a binding or the shooting controller will not shoot!");
    }
    ...
    private void ProcessInput()
    {
        if (isPlayerControlled)
        {
            if (fireAction.bindings.Count == 0)
                Debug.LogError(
                    "The Fire Input Action does not have a binding set! It must have a binding set in order to fire!");
            if (fireAction.ReadValue<float>() >= 1) Fire();
        }
    }
    ...
    public void Fire()
    {
        ...
        if (Time.timeSinceLevelLoad - lastFired > fireRate)
        {
            ...
            if (fireEffect != null) Instantiate(fireEffect, transform.position, transform.rotation, null);
    ...
    public void SpawnProjectile()
    {
        ...
        if (projectilePrefab != null)
        {
            ...
            var projectileGameObject = Instantiate(projectilePrefab, transform.position, transform.rotation, null);

            // Account for spread
            var rotationEulerAngles = projectileGameObject.transform.rotation.eulerAngles;
            ...
            if (projectileHolder != null) projectileGameObject.transform.SetParent(projectileHolder);
        }
    }
}