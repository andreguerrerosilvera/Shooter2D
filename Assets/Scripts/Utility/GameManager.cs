...
public class GameManager : MonoBehaviour
{
    ...
    [Tooltip(
        "Whether or not to print debug statements about whether the game can be won or not according to the game manager's" +
        " search at start up")]
    ...
    private void Awake()
    {
        ...
        if (player == null && FindObjectOfType<Controller>() != null)
            ...
            Debug.Log(
                "Player is not set and cannot find it in the scene. This is not a problem in non-playable scenes, such as the Main Menu.");
    }
    ...
    {
        var enemySpawners = FindObjectsOfType<EnemySpawner>().ToList();
        var staticEnemies = FindObjectsOfType<Enemy>().ToList();

        var numberOfInfiniteSpawners = 0;
        var enemiesFromSpawners = 0;
        var enemiesFromStatic = staticEnemies.Count;
        ...
        if (gameIsWinnable)
        {
            if (numberOfInfiniteSpawners > 0)
                Debug.Log("There are " + numberOfInfiniteSpawners + " infinite spawners " +
                          " so the level will always be winnable, "
                          + "\nhowever you sshould still playtest for timely completion");
            else if (enemiesToDefeat > numberOfEnemiesFoundAtStart)
                Debug.LogWarning("There are " + enemiesToDefeat + " enemies to defeat but only " +
                                 numberOfEnemiesFoundAtStart +
    ...
    {
        enemiesDefeated++;
        if (enemiesDefeated >= enemiesToDefeat && gameIsWinnable) LevelCleared();
    }
