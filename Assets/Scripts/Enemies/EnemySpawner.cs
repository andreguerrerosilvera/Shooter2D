...
{
    [Header("GameObject References")] [Tooltip("The enemy prefab to use when spawning enemies")]
    public GameObject enemyPrefab;

    [Tooltip("The target of the spwaned enemies")]
    public Transform target;

    [Header("Spawn Position")] [Tooltip("The distance within which enemies can spawn in the X direction")] [Min(0)]
    public float spawnRangeX = 10.0f;

    [Tooltip("The distance within which enemies can spawn in the Y direction")] [Min(0)]
    public float spawnRangeY = 10.0f;

    [Header("Spawn Variables")] [Tooltip("The maximum number of enemies that can be spawned from this spawner")]
    ...
    private void SpawnEnemy(Vector3 spawnLocation)
    {
        ...
        if (enemyPrefab != null)
        {
            ...
            var enemyGameObject = Instantiate(enemyPrefab, spawnLocation, enemyPrefab.transform.rotation, null);
            var enemy = enemyGameObject.GetComponent<Enemy>();
            var shootingControllers = enemyGameObject.GetComponentsInChildren<ShootingController>();

            // Setup the enemy if necessary
            if (enemy != null) enemy.followTarget = target;
            foreach (var gun in shootingControllers) gun.projectileHolder = projectileHolder;
    ...
    protected virtual Vector3 GetSpawnLocation()
    {
        ...
        var x = Random.Range(0 - spawnRangeX, spawnRangeX);
        var y = Random.Range(0 - spawnRangeY, spawnRangeY);
