    ...
    private void HandleDeathWithLives()
    {
        ...
        else
        {
            if (gameObject.name == "Player" && gameObject.tag != "Player")
                Debug.LogWarning(
                    "It looks like you're trying to kill a player, but your player hasn't been tagged as 'Player' in the inspector! \n Please tag your player.");
            if (gameObject.tag == "Player" && GameManager.instance != null) GameManager.instance.GameOver();
            if (gameObject.GetComponent<Enemy>() != null) gameObject.GetComponent<Enemy>().DoBeforeDestroy();
            Destroy(gameObject);
        }
    }
