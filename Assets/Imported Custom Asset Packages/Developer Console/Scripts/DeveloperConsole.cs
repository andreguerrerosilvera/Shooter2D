...
public class DeveloperConsole : MonoBehaviour
{
    ...
    [Header("Needed In Scene References")] public Text outputText;

    public InputField commandInputField;
    public EventSystem backUpEventSystem;
    public GameObject devLogCanvas;

    [Header("List of Console Commands")] public List<DeveloperConsoleCommand> developerConsoleCommands;
    ...
    public void Update()
    {
        if ((Keyboard.current.leftShiftKey.isPressed || Keyboard.current.rightShiftKey.isPressed) &&
            Keyboard.current.backquoteKey.isPressed && inputReleased)
        {
            ToggleDevConsole();
            inputReleased = false;
        }
        else if (!(Keyboard.current.leftShiftKey.isPressed || Keyboard.current.rightShiftKey.isPressed) ||
                 !Keyboard.current.backquoteKey.isPressed)
        {
            inputReleased = true;
        }
    }
    ...
    {
        var commandAsArray = commandInputField.text.Split().ToList();
        ...
        {
            foreach (var consoleCommand in developerConsoleCommands)
        ...
        {
            var invalidCommand = "";
            foreach (var commandPart in commandAsArray) invalidCommand += "|" + commandPart + "| ";
    ...
    private void Help(List<string> commandAsArray)
    {
        if ((commandAsArray[0] == "?" || commandAsArray[0] == "Help") && commandAsArray.Count == 1)
        {
            ...
            foreach (var consoleCommand in developerConsoleCommands) helpOutput += consoleCommand.OutputHelp() + "\n";
    ...
    private string OutputHelpFailure()
    {
        var failureMessage = "Invalid Use of command |?| or |Help| \n\n";
        failureMessage += OutputHelpHelp();
        return failureMessage;
    }

    private string OutputHelpHelp()
    {
        var helpMessage = "|?| or |Help|\n\n" +
